...
func main() {
	...
	// Download throughput (3 runs, 100MB file)
	fmt.Println("=== Download Throughput (3 runs) ===")
	dlURL := "https://proof.ovh.net/files/100Mb.dat"
	var speeds []float64
	for run := 1; run <= 3; run++ {
		fmt.Printf("Run %d: downloading 100MB... ", run)
		start := time.Now()
	...
	directClient := &http.Client{Timeout: 120 * time.Second}
	start := time.Now()
